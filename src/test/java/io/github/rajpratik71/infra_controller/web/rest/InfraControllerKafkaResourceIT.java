...
import io.github.rajpratik71.infra_controller.config.KafkaProperties;
...
class InfraControllerKafkaResourceIT {
    ...
    void setup() {
        ...
        InfraControllerKafkaResource kafkaResource = new InfraControllerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/infra-controller-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/infra-controller-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
